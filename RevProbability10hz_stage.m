...
for F=1:length(files)
    ...
    clearvars -except F exp  home name files bearing dBearing speed_ctr...
        HeadSens HeadSpeed HeadSpeedNet reversals rate gradient sensPath boundary
    cc=0;
    ...
    for T= 1:length(Tracks)
        ...
        if  Tracks(1,T).Frames(end)>3000 && Tracks(1,T).Frames(1)>1000
           ...
           %remove the ones outside  boundaries;
                for i=1:length(TX)
                    d(i)=abs(sqrt(((800-TX(i)).^2)+((800-TY(i)).^2)));
                end
                bi=find(d>boundary);
                TX(bi)=NaN;
                TY(bi)=NaN;
...
save(['revinfo_10hz_JR' fname(1:end-42)  name] , 'bearing' ,'dBearing' ,'speed_ctr','reversals','rate');
