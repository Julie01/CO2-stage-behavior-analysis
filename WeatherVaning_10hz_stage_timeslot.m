...
for F=1:length(files)
    ...
    HeadSens HeadSpeed HeadSpeedNet max_time fo folders
   cc=0;
    ...
    for T= 1:length(Tracks)
        ...
        % (ca frame 500)
        if  Tracks(1,T).Frames(end)>100 & Tracks(1,T).Frames(1)<max_time
            ...
            d=[];
            for i=1:length(RunY)
                d(i)=abs(sqrt(((800-RunX(i)).^2)+((800-RunY(i)).^2)));
            end
            bi=find(d>boundary);
            RunX(bi)=NaN;
            RunY(bi)=NaN;
...
%%  preferred 02 isocline:------
            ...
            if T>150 && T<170
                ...
                for R=1:length(RunX1)
                    try
                        scatter (RunX1{R}(1:end),RunY1{R}(1:end),30,Ctr_Bearing{T,R},'filled')
...
save(['runinfo_' fname(1:end-41)  name '_to' num2str(max_time/600) 'min'] , ...
    'bearing' ,'dBearing' ,'speed_ctr');
